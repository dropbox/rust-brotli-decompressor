...
links the reference C implementation into the fuzz binary and checks, on
every input: that any stream the C encoder produces with dictionaries
attached decodes byte-identically; that mutated serialized dictionaries are
accepted/rejected identically by both decoders; and that mutated or
truncated streams produce identical success/failure verdicts and outputs:

```
cd fuzz
BROTLI_C_ROOT=/path/to/google-brotli \
