...
fn DecodeWindowBits<Decoder:EntropyDecoder, Encoder:EntropyEncoder, AllocU32:Allocator<u32>>(
...
fn ReadSymbolCodeLengths<AllocU8: alloc::Allocator<u8>,
  ...
  while (symbol < alphabet_size && space > 0) {
    let mut p_index = 0;
    let code_len: u32;
    if (!bit_reader::BrotliCheckInputAmount(&s.br, bit_reader::BROTLI_SHORT_FILL_BIT_WINDOW_READ)) || !s.entropy_decoder.sufficient_bits(32) {
      s.symbol = symbol;
      s.repeat = repeat;
    ...
    if (code_len < kCodeLengthRepeatCode) {
      ...
      let repeat_delta: u32 = bit_reader::BrotliGetBitsUnmasked(&s.br) as u32 & bit_reader::BitMask(extra_bits);
  ...
  loop {
    match s.substate_huffman {
      BrotliRunningHuffmanState::BROTLI_STATE_HUFFMAN_NONE => {
        let (a_sub_loop_counter, a_result) = s.entropy_decoder.get_uniform(2, input, Unconditional{});
        if ANS_READER {
          if let BrotliResult::ResultSuccess = a_result {
      ...
      BrotliRunningHuffmanState::BROTLI_STATE_HUFFMAN_SIMPLE_SIZE => {
        let (a_symbol, a_result) = s.entropy_decoder.get_uniform(2, input, Unconditional{});
        if ANS_READER {
          if let BrotliResult::ResultSuccess = a_result {
            s.symbol = a_symbol;
...
fn SafeReadBlockLengthIndex<Decoder:EntropyDecoder, Encoder:EntropyEncoder, AllocU8: Allocator<u8>, AllocU32:Allocator<u32>, Speculative:BoolTrait>(
    ...
    match s.substate_context_map {
      BrotliRunningContextMapState::BROTLI_STATE_CONTEXT_MAP_NONE => {
        result = DecodeVarLenUint8(&mut s.substate_decode_uint8,
                                   &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32, num_htrees, input);
        match result {
          BrotliResult::ResultSuccess => {}
          _ => return result,
        }
...
fn DecodeBlockTypeAndLength<
  ...
  if (!safe) {
    ...
    fast_mut!((s.block_length)[tree_type as usize]) =
      ReadBlockLength(fast_slice!((s.block_len_trees)[tree_offset;]), &s.block_len_ans_table[tree_type as usize], br, decoder, encoder, m32, input);
    if ANS_READER {
    ...
    let index_ret = SafeReadBlockLengthIndex(&s.substate_read_block_length,
                                             ...
                                             input,
                                             Speculative{});
    if !SafeReadBlockLengthFromIndex(s, br, decoder, encoder, m32, &mut block_length_out, index_ret, input, Speculative{}) {
      s.substate_read_block_length =
...
fn DecodeLiteralBlockSwitchInternal<AllocU8: alloc::Allocator<u8>,
  ...
  if !DecodeBlockTypeAndLength(safe, &mut s.block_type_length_state, &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32, 0, input) {
    return false;
  }
...
fn DecodeCommandBlockSwitchInternal<AllocU8: alloc::Allocator<u8>,
  ...
  (safe: bool,
   ...
   input: &[u8])
   -> bool {
  if (!DecodeBlockTypeAndLength(safe, &mut s.block_type_length_state, &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32, 1, input)) {
    return false;
  }
...
fn DecodeDistanceBlockSwitchInternal<AllocU8: alloc::Allocator<u8>,
  ...
  (safe: bool,
   ...
   input: &[u8])
   -> bool {
  if (!DecodeBlockTypeAndLength(safe, &mut s.block_type_length_state, &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32, 2, input)) {
    return false;
  }
...
pub fn ReadDistanceInternal<AllocU8: alloc::Allocator<u8>,
  ...
  if (!safe) {
    ...
    let (a_distance_code, _) = s.entropy_decoder.get(distance_hgroup,
                                                     &s.distance_ans_table,
                                                     s.dist_htree_index as u8,
                                                     preloaded,
                                                     input,
  ...
  if (distval >= 0) {
    ...
    if (!safe && (s.distance_postfix_bits == 0)) {
      ...
      let mut dextra = bit_reader::BrotliReadBits(&mut s.br, nbits, input);
...
pub fn ReadCommandInternal<AllocU8: alloc::Allocator<u8>,
  ...
  if (!safe) {
    a_memento = s.entropy_decoder.placeholder();
    let (a_cc, _) = s.entropy_decoder.get(insert_copy_hgroup, &s.insert_copy_ans_table, s.htree_command_index as u8, preloaded, input, Unconditional{});
    if ANS_READER {
    ...
    let (a_cmd_code, a_result) = s.entropy_decoder.get(insert_copy_hgroup, &s.insert_copy_ans_table, s.htree_command_index as u8, preloaded, input, Speculative{});
    if let BrotliResult::NeedsMoreInput = a_result {
      s.entropy_decoder.abort_speculative(a_memento);
      return false;
    }
    ...
    loop {
      ...
      match s.state {
        BrotliRunningState::BROTLI_STATE_UNINITED => {
          ...
          // Decode window size.
          /* Reads 1..8 bits. */
          result = DecodeWindowBits(&mut s.large_window, &mut s.window_bits, &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32);
          match result {
            BrotliResult::ResultSuccess => {}
            _ => break,
          }
        ...
        BrotliRunningState::BROTLI_STATE_HUFFMAN_CODE_3 => {
          ...
          ind_ret = SafeReadBlockLengthIndex(&s.block_type_length_state.substate_read_block_length,
                                             ...
                                             &mut s.br, &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32, local_input, Unconditional{});

          if !SafeReadBlockLengthFromIndex(&mut s.block_type_length_state,
                                           &mut s.br,
                                           &mut s.entropy_decoder, &mut s.entropy_encoder, &mut s.alloc_u32,
